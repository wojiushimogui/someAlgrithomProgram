...
void numTimesInsortArray(int * arr,int n,int *findNumArray,int m) {
	...
	while(index<m){
		...
		//我们可以先用二分查找的方式来判断此数是否在此数组中，如存在，然后在向前和向后统计此数出现的次数 
		int begin=0;
		int end=n-1;
		int mid=0; 
		while(begin<=end){
			 mid=begin+(end-begin)/2;
			if(arr[mid]==findNumArray[index]){
				break;
			}
			else if(arr[mid]>findNumArray[index]){
				end=mid-1;
			}
			else{
				begin=mid+1;
		...
		if(begin<=end){//如果此条件满足，则说明此数在数组中一定是存在的,则先 result[index]++
			result[index]++; 
			//然后开始向前和向后统计
